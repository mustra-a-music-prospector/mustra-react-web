    ...
    return (
        <Container className="content-inner">
            ...
            <div className="item-row input-row">
                <WhiteInput hint={"가수이름"} value={artist} setValue={setArtist} />
            </div>
            <div className="item-row">
                <Button onClick={() => nextPage()}>Predict</Button>
            </div>
        </Container>
    );
};
const Page2 = ({ artist, song, setSong, prevPage }) => {
    return (
        <Container className="content-inner">
            <div className="header-row">
                <img id="logo" src={white_full_logo}></img>
            ...
            <div className="item-row input-row">
                <WhiteInput hint={"곡명"} value={song} setValue={setSong} />
            </div>
            <div className="item-row">
                <Button onClick={() => prevPage()}>Predict</Button>
            </div>
        </Container>
    );
};
...
class RenderNowPage extends React.Component {
    ...
    render() {
        const { artist, setArtist, song, setSong, page, nextPage, prevPage } = this.props;
        switch (page) {
            case 1:
                return <Page1 artist={artist} setArtist={setArtist} nextPage={nextPage} />;
            case 2:
                return <Page2 prevPage={prevPage} artist={artist} song={song} setSong={setSong} />;
            default:
                return <Page1 artist={artist} setArtist={setArtist} nextPage={nextPage} />;
        }
    }
